...
fetch('/users/getUserData', { credentials: 'include' })
    ...
    .then(async data => {
        if (data) {
            ...
            await caricaDati();
            creaMenuSx(livelloAccesso, "menuSx", data, caricaSetting, generaTabellaPazienti, settingUtente, gestisciChiusure, creaCardRepartoConLettiSVG, gestionePiano);
        }
    });
...
function generaTabellaPostiLiberi(IDUtente, idDivAggancio, livelloAccesso) {
    ...
    fetch(`/numeroLettiLiberi/${IDUtente}/${livelloAccesso}`)
        .then(res => res.json())
        .then(dati => {
...
function creaTabellaQuery(dati, colonne, idAggancio, titolo = null) {
...
async function generaTabellaPazienti(settings, idDivAggancio, livelloAccesso) {
    ...
    for (const settingID of settings) {
        ...
        if (livelloAccesso >= 50) {
            ...
            const response = await fetch(`/pazientiPerSetting/${settingID}`);
            pazienti = await response.json();
        }

        const responseSetting = await fetch(`/getSettingDestinazione`);
    ...
    <thead class="table table-primary">
        <tr>
            <th colspan="${mostraDestinazione ? 8 : 5}" class="text-center">${nomeSetting}</th>
        </tr>
        <tr>
            ${colSetting}
            <th>Nome</th>
            ...
            <th>Data Nascita</th>
            <th>Letto</th>
            ${mostraDestinazione ? "<th>Setting destinazione</th>" : ""}
...
`;
        ...
        pazienti.forEach(p => {
            ...
            selectSetting += `<option value="">Seleziona un valore...</option>`;

            settingData.forEach(s => {
                if (!settingsZona.includes(s.setting)) {
...
`;
            ...
            tr.querySelector(".btn-cancella").addEventListener("click", async () => {
                await eliminaPaziente(p.IDPaziente, p.IDPostoLetto, livelloAccesso);
            ...
            if (tr.querySelector('.btn-trasferisci')) {
                tr.querySelector('.btn-trasferisci').addEventListener('click', async (e) => {
                    ...
                    console.log(p.IDPaziente,IDSettingDestinazione,IDUtente, p.IDPostoLetto);
                    const response = await fetch(`/aggiornaDataTrasf/${p.IDPaziente}/${IDSettingDestinazione}/${IDUtente}/${p.IDPostoLetto}`);
                    await caricaSetting(IDUtente, livelloAccesso);
                    generaTabellaPazienti(settingUtente, "tabellaTrasf", livelloAccesso);
                    caricaStatoLetti();
                })
...
async function eliminaPaziente(IDPaziente, IDPostoLetto, livelloAccesso) {
    const responce = await fetch(`/eliminaPaziente/${IDPaziente}/${IDPostoLetto}/${livelloAccesso}`);
}
/*
...
async function caricaSetting(IDUtente, livelloAccesso) {
    try {
        ...
        reparto.forEach(setting => {
            ...
            aggancio.appendChild(creaReparto(setting.setting, setting.IDSetting));
...
function creaReparto(nome, IDSetting) {
    ...
    let pulsantiLetto = "";
    if (livelloAccesso >= 50) {
        pulsantiLetto = `<svg width="45" height="30" viewBox="0 0 45 30"
     xmlns="http://www.w3.org/2000/svg"
     class="piu svg-button">
  ...
  <!-- pulsante PIÙ -->
  <g transform="translate(32,7.5)" class="piu">
  ...
  <!-- pulsante MENO -->
  <g transform="translate(32,7.5)" class="meno">
    <circle cx="5" cy="5" r="5"
            fill="#000000" stroke="#000000" stroke-width="1"
            class="meno"/>
...
document.addEventListener('click', async (e) => {
        ...
        if (btnPiu) {
            ...
            const response = await fetch(`/aggiuntaBoarding/4`);
        ...
        if (btnMeno) {
            ...
            const response = await fetch(`/sottraiBoarding/4`);
            if (!response.ok) throw new Error("non ho ricevuto i dati del letto");
